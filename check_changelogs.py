    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if app_frame is not None:
                ...
                page.wait_for_timeout(4000)
                app_snippet = re.sub(
                    r"\s+", " ", app_frame.inner_text("body")
                )[:400]
                child_urls = [
                    f.url for f in app_frame.child_frames
                ]
                print(
                    f"[Diagnose] {label}: iframe-Inhalt (Anfang): "
                    f"{app_snippet!r} | Verschachtelte Frames: {child_urls}",
                    file=sys.stderr,
                )
